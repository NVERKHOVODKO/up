...
using Coin = Up.Core.Models.Coin;
using Exception = System.Exception;
using Transactions = Up.Core.Models.Transactions;
using Withdrawal = Up.Core.Models.Withdrawal;

namespace Up.Infrastructure.Repositories;
...
public class CurrencyRepository : RepositoryBase, ICurrencyRepository
{
    ...
    public CurrencyRepository(AppDbContext context, IDbRepository dbRepository)
    {
        _context = context;
    ...
    public void AddCryptoToUserWallet(Guid userId, string shortname, double quantity)
    {
        ...
        if(user?.Login is "Service" || user is null)
            return;
    ...
    public async Task<CoinsInformation> GetFullCoinInformation(string shortName)
    {
        ...
        return new CoinsInformation(fullName, shortName,
            @"C:\НЕ СИСТЕМА\BSUIR\второй курс\UP.Api\cryptoicons_png\128\" + shortName.ToLower(), dailyVolume, priceChange,
